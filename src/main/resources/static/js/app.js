(() => {
    ...
    function overview(id) {
        Promise.all([mods.products.listFn(), mods.warehouses.listFn(), mods.inventory.listFn()]).then(([p, w, i]) => {
            if (id !== renderSeq) return;
            root().innerHTML = `<section class="overview-panel section-stack"><div class="panel__header"><div><p class="panel__eyebrow">Overview</p><h3 class="panel__title">Live module snapshot</h3><p class="panel__copy">All module counts are loaded from the protected APIs using the stored JWT.</p></div><div class="helper-chip-row"><span class="helper-chip">GET /products</span><span class="helper-chip">GET /warehouses</span><span class="helper-chip">GET /inventory</span></div></div><div class="metric-grid">${metric("Products", p.length, "Catalog records")}${metric("Warehouses", w.length, "Storage locations")}${metric("Inventory", i.length, "Stock records")}</div></section><div class="view-grid view-grid--split"><section class="panel"><div class="panel__header"><div><p class="panel__eyebrow">Modules</p><h3 class="panel__title">Navigate by workflow</h3><p class="panel__copy">Each module includes list, detail, create, and edit routes.</p></div></div><div class="overview-grid">${moduleCard(mods.products, p.length, "Manage SKUs, pricing, and descriptions.")}${moduleCard(mods.warehouses, w.length, "Review warehouse records and update locations.")}${moduleCard(mods.inventory, i.length, "Track stock by product and warehouse.")}</div></section>${overviewApi()}</div>`;
